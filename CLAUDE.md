...
### 테스트 실행 보고서 보관
- 날짜별 실행 보고서, TRX·로그, 제공자 응답과 계정별 진단은 `artifacts/` 또는 CI 산출물에 보관하고 커밋하지 않는다. 기존 로컬 `tests/Mythosia.AI.Test/validation/` 보고서도 Git 추적에서 제외한다.
- 미구현 설계 초안·작업 메모·미사용 이미지 초안은 `.artifacts/local-documents/` 같은 로컬 보관 폴더에 둔다. 임시 빌드·진단 폴더에 생긴 문서도 커밋하지 않는다.
- 재사용 가능한 테스트 코드·fixture·실행 방법·검토된 회귀 기준 데이터는 저장소에 유지한다. 공개 문서에는 필요한 검증 요약과 한계만 남기며 로컬 보고서로 연결하지 않는다.
