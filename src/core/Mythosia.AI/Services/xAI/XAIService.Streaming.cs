    ...
    public partial class XAIService
    {
        ...
        protected override OpenAIStreamChunk ParseStreamChunk(string jsonData, StreamOptions options)
        {
                ...
                finishReasonElement.ValueKind != JsonValueKind.Null)
            {
                ...
                {
                    chunk.Error = CreateCompatibleStreamError(
                        "xAI ended the stream before a complete tool response was available; no tools were executed.",
            ...
            StreamDiagnostics diagnostics)
        {
            return CreateCompatibleStreamError(
                "xAI emitted a malformed streaming response; no tools were executed.",
                "malformed_stream",
                exception.Message);
        }
            ...
            StreamDiagnostics diagnostics)
        {
            ...
            return CreateCompatibleStreamError(
                "xAI stream ended before the [DONE] marker; no tools were executed.",
                "incomplete_stream",
