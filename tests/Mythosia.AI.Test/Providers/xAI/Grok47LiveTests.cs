...
[TestCategory("Grok47")]
[DoNotParallelize]
public class Grok47LiveTests
{
    // Paid opt-in checks use synthetic prompts and images only. Missing access, actual-tier
    // downgrades and transport errors fail the test rather than being counted as skips.
    ...
    {
        using var probe = await Grok46LiveProbe.CreateAsync(AIModels.xAI.Grok4_7);
        ...
        Console.WriteLine($"LIVE_GROK47_OK feature=provider-reasoning level={level}");
    }
    ...
    public async Task CommonReasoning_WorksThroughStreamingAndRun(ReasoningLevel level, Grok46ExecutionMode mode)
    {
        ...
        Console.WriteLine($"LIVE_GROK47_OK feature=common-reasoning mode={mode} level={level}");
    }
    ...
    public async Task AutomaticClientFunction_PreservesActualCallIdAndReturnsHandlerOnlyData(Grok46ExecutionMode mode)
    {
        ...
        Console.WriteLine($"LIVE_GROK47_OK feature=client-function-correlation mode={mode}");
    }
    ...
    public async Task RagQueryRewrite_UsesLowFloorAndRestoresHighForFinalAnswer(bool useRun)
    {
        ...
        Console.WriteLine($"LIVE_GROK47_OK feature=rag-rewrite-low-floor-and-restore run={useRun}");
    }
    ...
    public async Task TypedJsonMode_ReturnsTheRequestedObjectWithoutRepair(bool streaming)
    {
        ...
        Console.WriteLine($"LIVE_GROK47_OK feature=typed-json-mode streaming={streaming}");
    }

    [TestMethod]
    [DataRow(Grok46ExecutionMode.LegacyCallback)]
    [DataRow(Grok46ExecutionMode.RichStream)]
    public async Task ProviderReasoning_LegacyStreamsKeepTheSelectedModel(Grok46ExecutionMode mode)
    {
        using var probe = await Grok46LiveProbe.CreateAsync(AIModels.xAI.Grok4_7);
        probe.Service.ReasoningEffort = GrokReasoning.Low;
        var answer = await probe.ExecuteAsync(Calculation, mode);
        Assert.AreEqual("208", answer.Text.Trim());
        Assert.AreEqual(1, probe.Requests.Count);
        AssertEffort(probe.Requests[0], "low");
    ...
    [TestMethod]
    [DataRow(InferenceSpeed.ProviderDefault, false)]
    [DataRow(InferenceSpeed.ProviderDefault, true)]
    [DataRow(InferenceSpeed.Standard, false)]
    [DataRow(InferenceSpeed.Standard, true)]
    [DataRow(InferenceSpeed.Fast, false)]
    [DataRow(InferenceSpeed.Fast, true)]
    public async Task RequestBuilder_SpeedReportsTheActualTier(InferenceSpeed speed, bool useRun)
    {
        using var probe = await Grok46LiveProbe.CreateAsync(AIModels.xAI.Grok4_7);
        using var timeout = new CancellationTokenSource(TimeSpan.FromMinutes(4));
        ...
        if (useRun)
        {
            ...
            await using var run = await request.StartRunAsync(chunk => chunks.Append(chunk),
                StreamOptions.FullOptions, timeout.Token);
            ...
            Assert.AreEqual(text, chunks.ToString());
            Assert.AreEqual(text, string.Concat(events.Where(item => item.Type == StreamingContentType.Text).Select(item => item.Content)));
        ...
        else
        {
            ...
            observations = probe.Service.LastProcessing;
        }
        StringAssert.Contains(text, "SPEED_OK");
        Assert.AreEqual(1, probe.Requests.Count);
        Assert.AreEqual(1, observations.Count);
        var expectedTier = speed switch { InferenceSpeed.Fast => "priority", InferenceSpeed.Standard => "default", _ => null };
        Assert.AreEqual(expectedTier, probe.Requests[0].Body["service_tier"]?.GetValue<string>());
        var actual = observations[0];
        Assert.AreEqual(speed, actual.RequestedSpeed);
        Assert.IsNotNull(actual.AppliedSpeed, "Actual processing must be reported by the server.");
        if (speed != InferenceSpeed.ProviderDefault)
            Assert.AreEqual(speed, actual.AppliedSpeed.Value, "A downgrade does not prove Fast succeeded.");
        Assert.IsFalse(string.IsNullOrWhiteSpace(actual.RawAppliedMode));
        AssertEffort(probe.Requests[0], "low");
        Assert.AreEqual(GrokReasoning.Auto, probe.Service.ReasoningEffort, "The builder must not mutate the service baseline.");
        probe.AssertTransport(streaming: useRun);
        Console.WriteLine($"LIVE_GROK47_SPEED run={useRun} requested={speed} actual={actual.RawAppliedMode}");
    }

    [TestMethod]
    public async Task Vision_RecognizesSyntheticImageWithoutModelSubstitution()
    {
        using var probe = await Grok46LiveProbe.CreateAsync(AIModels.xAI.Grok4_7);
        using var timeout = new CancellationTokenSource(TimeSpan.FromMinutes(4));
        probe.Service.ReasoningEffort = GrokReasoning.Low;
        var path = Path.Combine(Path.GetTempPath(), "mythosia-grok47-" + Guid.NewGuid().ToString("N") + ".png");
        try
        {
            ...
            await File.WriteAllBytesAsync(path, png.ToArray(), timeout.Token);
            var answer = await probe.Service.GetCompletionWithImageAsync(
                "What color fills this image? Reply with only the English color name.", path, timeout.Token);
            StringAssert.Contains(answer.ToLowerInvariant(), "blue");
            Assert.AreEqual(1, probe.Requests.Count);
            var parts = probe.Requests[0].Body["messages"]!.AsArray().OfType<JsonObject>()
                .Where(message => message["role"]?.GetValue<string>() == "user")
                .SelectMany(message => message["content"] is JsonArray content ? content.OfType<JsonObject>() : []);
            Assert.IsTrue(parts.Any(part => part["type"]?.GetValue<string>() == "image_url"));
            AssertEffort(probe.Requests[0], "low");
            probe.AssertTransport(streaming: false);
        }
