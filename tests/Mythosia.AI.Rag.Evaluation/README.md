...
보고서와 fingerprint용 JSON의 서식 줄바꿈은 LF로 고정하여 Windows와 Linux에서 같은 조건을 비교할 수 있습니다. 문서 본문 자체의 줄바꿈은 바꾸지 않습니다. 이 수정 전에 Windows에서 만든 기준선은 서식 차이로 비교에 실패할 수 있습니다. 코퍼스·벡터·질문별 순위와 지표가 같은지 확인한 뒤 기준선을 새 실행 결과로 갱신하세요. 저장소 smoke 기준선은 두 운영체제의 결과가 같은지 확인한 뒤 갱신했으며, 단위 테스트에서도 이 기준선과 직접 비교합니다.
