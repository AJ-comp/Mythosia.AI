...
.WithRag(rag => rag
    ...
    .WithScoreThreshold(0.6)       // Minimum similarity score
    .AddDocument("corpus.txt")
)
```
...
- **`WithScoreThreshold`** — discard anything below this similarity threshold, even if fewer than `TopK` chunks remain
