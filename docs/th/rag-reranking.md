...
> 📍 **Q&A Pipeline:** [การเขียนคำถามใหม่](rag-query-rewriting.md) → การกรอง → Embedding (เมื่อจำเป็น) → [การดึงข้อมูล](rag-hybrid-search.md) → **`Reranking`** → การสร้าง Context
...
เพื่อไม่ให้คำถามและเอกสารของการประเมินแต่ละครั้งปะปนกับการประเมินก่อนหน้าหรือบทสนทนาของ service นั้น `LlmReranker` จะส่งคำขอแบบ stateless แยกกันในแต่ละครั้ง โดยไม่อ่านหรือเพิ่มเนื้อหาลงในประวัติการสนทนา ค่าเริ่มต้นของ service และโค้ดที่ใช้เรียกยังคงเดิม การประเมินของ reranker ที่ใช้ AI service ตัวเดียวกันจะถูกประมวลผลทีละครั้งตามลำดับ
