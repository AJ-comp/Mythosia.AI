...
service.WithStructuredOutputPolicy(StructuredOutputPolicy.Strict);  // До 3 попыток автоисправления
service.WithStructuredOutputPolicy(StructuredOutputPolicy.NoRetry); // Без повторных попыток исправления
```

Число исправлений не включает первоначальный ответ. `StructuredOutputMaxRetries` и `MaxRepairAttempts` считают отрицательные значения нулём и допускают максимум `int.MaxValue - 1`. Значение `int.MaxValue` вызывает `ArgumentOutOfRangeException` до запроса к провайдеру, чтобы общее число попыток не переполнилось. Правило действует для типизированных вызовов завершения и `BeginStream(...).As<T>()`.
