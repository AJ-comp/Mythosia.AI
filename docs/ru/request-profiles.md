...
Профиль задаёт настройки одного вызова, но не гарантирует безопасное одновременное использование общего диалога. Рекомендуется `service.CreateRequest(prompt).WithProfile(profile)`. Существующие перегрузки поддерживаются.
