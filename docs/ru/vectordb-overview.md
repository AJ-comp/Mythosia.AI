...
public class VectorSearchResult
{
    ...
    public double Score { get; set; }  // Шкала зависит от бэкенда и режима поиска; больше — лучше
}
```
...
| Gt / Gte / Lt / Lte | Клиент | Клиент | **Сервер** | **SQL** |
| Like | Клиент | Клиент | Клиент | **SQL** |
| Exists / NotExists | Клиент | Клиент | Клиент | **SQL** |

Postgres обеспечивает полное SQL-делегирование для всех операторов. Qdrant и Pinecone нативно поддерживают равенство, проверку принадлежности множеству и операторы сравнения.

> **Примечание:** Qdrant молча игнорирует неподдерживаемые операторы фильтра (`Like`, `Exists`, `NotExists`) — они не применяются и на стороне клиента. Если эти операторы необходимы при работе с Qdrant, применяйте дополнительную фильтрацию к возвращённым результатам.
