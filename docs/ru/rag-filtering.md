...
> 📍 **Пайплайн вопрос-ответ:** [Переписывание запросов](rag-query-rewriting.md) → **`Фильтрация`** → [Эмбеддинг (при необходимости)](rag-embedding.md) → [Поиск](rag-hybrid-search.md) → [Переранжирование](rag-reranking.md) → [Построение контекста](rag-context-build.md)
