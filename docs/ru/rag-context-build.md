...
> 📍 **Пайплайн вопрос-ответ:** [Переписывание запросов](rag-query-rewriting.md) → [Фильтрация](rag-filtering.md) → [Эмбеддинг (при необходимости)](rag-embedding.md) → [Поиск](rag-hybrid-search.md) → [Переранжирование](rag-reranking.md) → **`Построение контекста`**
