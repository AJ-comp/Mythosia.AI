...
### Параллельное использование и изменение записей

При обновлении общего хранилища во время запроса текст и ключевой индекс должны соответствовать одной версии. `InMemoryVectorStore` синхронизирует запись, удаление и чтение, поэтому каждый векторный, текстовый или гибридный запрос видит согласованное состояние. Обе части гибридного запроса используют это же состояние.

Хранилище копирует входящие записи, включая массивы векторов и метаданные, и возвращает независимые копии при чтении, поиске и диагностике. Изменение входного объекта или полученной записи не меняет сохранённые данные; для сохранения изменений вызовите `UpsertAsync` повторно. Не изменяйте входные записи, векторы и метаданные, пока вызов копирует или читает их.

Переданный `CancellationToken` позволяет отменить вызов, пока тот ждёт освобождения блокировки хранилища другой операцией. Отмена ожидания сама по себе не прерывает операцию, уже использующую хранилище. После начала обновления записи отмена не прерывает его между обновлением текста и индекса.
