...
Кількість виправлень не включає початкову відповідь. `StructuredOutputMaxRetries` і `MaxRepairAttempts` вважають від’ємні значення нулем і допускають щонайбільше `int.MaxValue - 1`. Значення `int.MaxValue` спричиняє `ArgumentOutOfRangeException` до запиту до провайдера, щоб загальна кількість спроб не переповнилася. Це стосується типізованих викликів завершення та `BeginStream(...).As<T>()`.
