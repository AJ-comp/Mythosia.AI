...
### Паралельне використання та зміна записів

Під час оновлення спільного сховища одночасно із запитом текст та індекс ключових слів мають відповідати одній версії. `InMemoryVectorStore` синхронізує запис, видалення та читання, тому кожен векторний, текстовий або гібридний запит бачить узгоджений стан. Обидві частини гібридного запиту використовують цей самий стан.

Сховище копіює вхідні записи, включно з масивами векторів і метаданими, та повертає незалежні копії під час читання, пошуку й діагностики. Зміна вхідного об’єкта або отриманого запису не змінює збережені дані; для збереження змін повторно викличте `UpsertAsync`. Не змінюйте вхідні записи, вектори й метадані, поки виклик копіює або читає їх.

Переданий `CancellationToken` дає змогу скасувати виклик, поки він чекає звільнення блокування сховища іншою операцією. Скасування очікування саме по собі не перериває операцію, яка вже використовує сховище. Після початку оновлення запису скасування не перериває його між оновленням тексту та індексу.
...
> Для застосування заданих параметрів векторного пошуку за двох активних гілок гібридного пошуку потрібен `Mythosia.VectorDb.Postgres` 10.8.1 або новіший. [Примітки до виправлення](../../src/vectordb/Mythosia.VectorDb.Postgres/RELEASE_NOTES.md#v1081).

Область пошуку векторних кандидатів у гібридному пошуку задається через `HnswIndexOptions.EfSearch` або `IvfFlatIndexOptions.Probes` у `PostgresOptions.Index`. Ці типові значення застосовуються до звичайного векторного пошуку та векторної частини гібридного пошуку в тій самій транзакції, що й пошуковий запит. Наведені вище перевизначення параметрів для окремого запиту доступні лише в `SearchAsync`. Наближений пошук і фільтри все одно можуть повертати менше ніж `topK` результатів.
