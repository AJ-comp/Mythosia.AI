...
Профіль задає налаштування одного виклику, але не гарантує безпечне одночасне використання спільної розмови. Рекомендовано `service.CreateRequest(prompt).WithProfile(profile)`. Наявні перевантаження підтримуються.
