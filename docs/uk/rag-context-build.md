...
> 📍 **Пайплайн запитання-відповіді:** [Переписування запитів](rag-query-rewriting.md) → [Фільтрація](rag-filtering.md) → [Ембеддинг (за потреби)](rag-embedding.md) → [Пошук](rag-hybrid-search.md) → [Переранжування](rag-reranking.md) → **`Побудова контексту`**
