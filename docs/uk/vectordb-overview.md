...
public class VectorSearchResult
{
    ...
    public double Score { get; set; }  // Шкала залежить від бекенду та режиму пошуку; більше — краще
}
```
...
| Gt / Gte / Lt / Lte | Клієнт | Клієнт | **Сервер** | **SQL** |
| Like | Клієнт | Клієнт | Клієнт | **SQL** |
| Exists / NotExists | Клієнт | Клієнт | Клієнт | **SQL** |

Postgres забезпечує повне SQL-делегування для всіх операторів. Qdrant та Pinecone нативно підтримують рівність, перевірку належності множині та оператори порівняння.

> **Примітка:** Qdrant мовчки ігнорує непідтримувані оператори фільтра (`Like`, `Exists`, `NotExists`) — вони не застосовуються і на боці клієнта. Якщо ці оператори потрібні при роботі з Qdrant, застосуйте додаткову фільтрацію до повернутих результатів.
