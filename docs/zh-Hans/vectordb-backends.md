...
查询共享存储期间更新文档时，正文与关键词索引必须对应同一版本。`InMemoryVectorStore` 会同步写入、删除和读取，使每次向量、文本或混合查询都读取一致的状态。混合查询的两个检索分支也使用同一状态。

存储会复制传入的记录，包括向量数组和元数据；按 ID 获取、搜索和诊断返回的记录也是独立副本。仅修改输入对象或返回的记录不会更改已存储的数据；请再次调用 `UpsertAsync` 保存修改。 调用正在复制或读取输入时，请勿修改输入记录、向量或元数据。

等待其他操作释放存储锁时，也可使用传入的 `CancellationToken` 取消调用。取消等待本身不会中止正在使用存储的其他操作。单条记录的正文与索引更新开始后，取消不会在两者之间打断更新。

取消批量写入后，已经写入的记录可能保留。`ReplaceByFilterAsync` 仍按顺序执行删除和批量插入，不使用事务：其他查询可能看到中间的空缺，失败或取消也不会回滚已完成的写入。
...
> 要在混合检索的两条搜索路径均启用时应用配置的向量搜索参数，需要 `Mythosia.VectorDb.Postgres` 10.8.1 或更高版本。[补丁说明](../../src/vectordb/Mythosia.VectorDb.Postgres/RELEASE_NOTES.md#v1081)。

要调整混合检索中向量候选的搜索范围，请在 `PostgresOptions.Index` 中设置 `HnswIndexOptions.EfSearch` 或 `IvfFlatIndexOptions.Probes`。这些默认值会在搜索查询的同一事务内应用于普通向量搜索和混合搜索的向量分支。上面的按请求运行时覆盖仅适用于 `SearchAsync`。近似搜索和过滤仍可能使结果数少于 `topK`。
