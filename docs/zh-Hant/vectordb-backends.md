...
查詢共用儲存區期間更新文件時，本文與關鍵字索引必須對應同一版本。`InMemoryVectorStore` 會同步寫入、刪除和讀取，使每次向量、文字或混合查詢都讀取一致的狀態。混合查詢的兩個檢索分支也使用同一狀態。

儲存區會複製傳入的記錄，包括向量陣列和中繼資料；依 ID 取得、搜尋和診斷回傳的記錄也是獨立副本。僅修改輸入物件或回傳的記錄不會變更已儲存的資料；請再次呼叫 `UpsertAsync` 儲存修改。 呼叫正在複製或讀取輸入時，請勿修改輸入記錄、向量或中繼資料。

等待其他操作釋放儲存區鎖定時，也可使用傳入的 `CancellationToken` 取消呼叫。取消等待本身不會中止正在使用儲存區的其他操作。單筆記錄的本文與索引更新開始後，取消不會在兩者之間打斷更新。
