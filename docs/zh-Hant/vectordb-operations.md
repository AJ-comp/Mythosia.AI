...
上面的多載使用各後端既有的混合檢索行為。目前原始碼中的 InMemory、PostgreSQL 和 Qdrant 也實作了 `ITextSearchStore` 與 `IConfigurableHybridSearchStore`。這些選用 API **尚未發布（Unreleased）**；Pinecone 未實作它們。
