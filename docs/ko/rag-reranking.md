...
> 📍 **질문 응답 파이프라인:** [쿼리 재작성](rag-query-rewriting.md) → 필터링 → 임베딩(필요 시) → [검색](rag-hybrid-search.md) → **`재순위`** → 컨텍스트 구성
...
각 평가의 질문과 문서가 이전 평가나 서비스의 대화 내용과 섞이지 않도록, `LlmReranker`는 평가마다 이력을 사용하지 않는 독립 요청을 보냅니다. 기존 대화 이력을 읽거나 평가 내용을 이력에 추가하지 않습니다. 서비스의 기본 설정과 사용자 호출 방식은 그대로 유지됩니다. 같은 AI 서비스를 공유하는 재순위기들의 평가는 순서대로 처리됩니다.
