...
ほとんどのAIプロバイダーSDKは異なるAPIを公開しているため、プロバイダーの切り替えや機能の組み合わせが困難です。Mythosia.AIはそれらを単一の`IAIService`インターフェースでラップするため、どのモデルやプロバイダーを使用しても、アプリケーションコードは同じままです。
...
| ステップ | パッケージ | 用途 |
|:------:|---------|------|
| **1** | `Mythosia.AI` | 開始点 — 補完、ストリーミング、関数呼び出し、構造化出力 |
