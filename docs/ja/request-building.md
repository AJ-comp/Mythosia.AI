...
画面で回答を待つ利用者には有料の低遅延処理を、バックグラウンドの報告書には通常処理を選べます。`WithSpeed` はモデルと推論レベルを保ったまま処理モードを指定します。未公開機能で、対応する core と abstractions の変更が必要です。公開済み 8.0.0 / 4.0.0 には含まれません。
