...
共有ストアを検索中に更新しても、本文とキーワードインデックスは同じ版を示す必要があります。`InMemoryVectorStore` は保存・削除・読み取りを同期し、ベクトル検索、テキスト検索、ハイブリッド検索がそれぞれ一貫した状態を参照するようにします。ハイブリッド検索の両方の検索経路も同じ状態を使用します。

保存時にレコード、ベクトル配列、メタデータをコピーし、取得・検索・診断の結果も独立したコピーとして返します。入力オブジェクトや返されたレコードを変更するだけでは保存内容は変わりません。変更を保存するには `UpsertAsync` を再度呼び出してください。 呼び出しが入力をコピーまたは読み取っている間は、入力レコード・ベクトル・メタデータを変更しないでください。

別の操作がストアのロックを解放するのを待っている間も、渡した `CancellationToken` で呼び出しをキャンセルできます。待機のキャンセル自体が、ストアを使用中の別の操作を中断することはありません。開始済みのレコードの本文とインデックスの更新を、キャンセルによって途中で分断することはありません。
