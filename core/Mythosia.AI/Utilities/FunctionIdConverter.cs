...
namespace Mythosia.AI.Utilities
{
    ...
    public static class FunctionIdConverter
    {
        ...
        /// </summary>
        public static string ConvertId(string originalId, IdSource source, string targetProvider)
        {
            return targetProvider switch
            {
                nameof(AIProvider.OpenAI) => ToOpenAIId(originalId, source),
                nameof(AIProvider.Anthropic) => ToClaudeId(originalId, source),
                nameof(AIProvider.Google) => ToGeminiId(originalId, source),
