...
$resultsDirectory = Join-Path $repoRoot "artifacts/test-results/inference-speed-live"
$reportFileName = "inference-speed-live-$([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss-fff'))-$([Guid]::NewGuid().ToString('N')).trx"
...
$testArguments = @(
    ...
    "--filter", "FullyQualifiedName~InferenceSpeedLiveTests&TestCategory=Live",
...
Write-Host "Running Cross-provider inference speed live contracts using existing Key Vault credentials. These calls incur API charges."
Write-Host "TRX report: $reportPath"
Push-Location -LiteralPath $repoRoot
try {
    New-Item -ItemType Directory -Path $resultsDirectory -Force | Out-Null
    & dotnet @testArguments
    if ($LASTEXITCODE -ne 0) { throw "Cross-provider inference speed live tests failed (exit code $LASTEXITCODE)." }
    ...
    if ($total -lt $minimumExpectedTests -or $executed -ne $total -or $passed -ne $total -or
        ...
        throw "Every live case must pass without skipping: total=$total executed=$executed passed=$passed skipped=$skipped notExecuted=$notExecuted inconclusive=$inconclusive. Report: $reportPath"
    }
    Write-Host "Cross-provider inference speed live validation passed: $passed/$total tests, none skipped."
}
