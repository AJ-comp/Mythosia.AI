...
$minimumExpectedTests = 16
$resultsDirectory = Join-Path $repoRoot "artifacts/test-results/anthropic-fable-live"
$reportFileName = "anthropic-fable-live-$([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss-fff'))-$([Guid]::NewGuid().ToString('N')).trx"
...
$testArguments = @(
    ...
    "--filter", "FullyQualifiedName~AnthropicFable51LiveTests&TestCategory=Live",
...
Write-Host "Running Claude Fable 5.1 live contracts using existing Key Vault credentials. These calls incur API charges."
Write-Host "TRX report: $reportPath"
Push-Location -LiteralPath $repoRoot
try {
    New-Item -ItemType Directory -Path $resultsDirectory -Force | Out-Null
    & dotnet @testArguments
    if ($LASTEXITCODE -ne 0) { throw "Claude Fable 5.1 live tests failed (exit code $LASTEXITCODE)." }
    ...
    Write-Host "Claude Fable 5.1 live validation passed: $passed/$total tests, none skipped."
}
