...
$resultsDirectory = Join-Path $repoRoot "artifacts/test-results/openai-gpt6-sol-luna-live"
$reportFileName = "openai-gpt6-sol-luna-live-$([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss-fff'))-$([Guid]::NewGuid().ToString('N')).trx"
...
$testArguments = @(
    ...
    "--filter", "FullyQualifiedName~OpenAIGpt6SolLunaLiveTests&TestCategory=Live",
...
Write-Host "Running GPT-6 Sol/Luna live contracts using existing Key Vault credentials. These calls incur API charges."
Write-Host "TRX report: $reportPath"
Push-Location -LiteralPath $repoRoot
try {
    New-Item -ItemType Directory -Path $resultsDirectory -Force | Out-Null
    & dotnet @testArguments
    if ($LASTEXITCODE -ne 0) { throw "GPT-6 Sol/Luna live tests failed (exit code $LASTEXITCODE)." }
    ...
    Write-Host "GPT-6 Sol/Luna live validation passed: $passed/$total tests, none skipped."
}
