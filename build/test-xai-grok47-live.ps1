...
$expectedTests = 25
$resultsDirectory = Join-Path $repoRoot "artifacts/test-results/xai-grok47-live"
$reportFileName = "xai-grok47-live-$([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss-fff'))-$([Guid]::NewGuid().ToString('N')).trx"
...
$testArguments = @(
    ...
    "--filter", "FullyQualifiedName~Grok47LiveTests&TestCategory=Live",
...
Write-Host "Running Grok 4.7 live contracts using the existing xAI Key Vault credential. These real model calls incur API charges."
Write-Host "TRX report: $reportPath"
Push-Location -LiteralPath $repoRoot
try {
    New-Item -ItemType Directory -Path $resultsDirectory -Force | Out-Null
    & dotnet @testArguments
    if ($LASTEXITCODE -ne 0) { throw "Grok 4.7 live tests failed (exit code $LASTEXITCODE)." }
    ...
    $requests = @($lines | Where-Object { $_.StartsWith("LIVE_GROK47_REQUEST ") } |
        ForEach-Object { $_.Substring("LIVE_GROK47_REQUEST ".Length) | ConvertFrom-Json })
    if ($requests.Count -ne 29 -or @($requests | Where-Object { $_.StatusCode -ne 200 -or $_.model -ne "grok-4.7" }).Count -ne 0) {
        throw "Expected exactly 29 successful real grok-4.7 requests, observed $($requests.Count). Report: $reportPath"
    }
    $expectedEfforts = @{ low = 15; medium = 6; high = 4; xhigh = 2 }
    ...
    Write-Host "Grok 4.7 live validation passed: $passed/$total tests, 29 successful model requests, every supported reasoning effort verified, none skipped."
}
