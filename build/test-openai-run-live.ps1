...
$resultsDirectory = Join-Path $repoRoot "artifacts/test-results/openai-run-live"
$reportFileName = "openai-run-live-$([DateTime]::UtcNow.ToString('yyyyMMdd-HHmmss-fff'))-$([Guid]::NewGuid().ToString('N')).trx"
...
$testArguments = @(
    ...
    "--filter", "FullyQualifiedName~OpenAIRunLiveTests&TestCategory=Live",
...
Write-Host "Running OpenAI Run and steering live tests using the existing Key Vault credentials. These calls incur API charges."
Write-Host "TRX report: $reportPath"
Push-Location -LiteralPath $repoRoot
try {
    New-Item -ItemType Directory -Path $resultsDirectory -Force | Out-Null
    & dotnet @testArguments
    if ($LASTEXITCODE -ne 0) { throw "OpenAI Run live tests failed (exit code $LASTEXITCODE)." }
    ...
    if ($total -lt $minimumExpectedTests -or $executed -ne $total -or $passed -ne $total -or
        ...
        throw "Live validation requires every case to pass with none skipped. total=$total executed=$executed passed=$passed skipped=$skipped notExecuted=$notExecuted inconclusive=$inconclusive. Report: $reportPath"
    }
    Write-Host "OpenAI Run live validation passed: $passed/$total tests, none skipped."
}
